...
class Test:
    ...
    @limiter.limit("5/second")
    async def bmi_calc(
        weight, heightft, heightin, request: Request, response: Response
    ):
        if await utils.var_can_be_type(weight, float) and await utils.var_can_be_type(
            heightft, float
        ):
            if heightin == "":
                heightin = 0
...
class api_class:
        ...
        async def contact_api(
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    return HTMLResponse(
                        await response.read(), status_code=response.status
                    )

        @api.post("/freelance", include_in_schema=False)
        @limiter.limit("5/second")
        async def freelance_api(
            response: Response,
            request: Request,
            name: str = Form(None),
            email: str = Form(None),
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    "http://jglbotapi.us:83/freelance",
                    json={
                        "ip": request.client.host,
                        "name": name,
                        "email": email,
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get(
                        ...
                        headers={f"User-Agent": "keycdn-tools:http://{ip}"},
                    ) as res:
                        data = await res.json()
                        for x in data.get("data").get("geo"):
                            if (
                                data.get("data").get("geo").get(x) is None
                                or data.get("data").get("geo").get(x) == ""
                            ):
    ...
    class Bot:
        @api.get(
            "/bot/status", description="Checks if the JGL Bot is online or offline"
        )
        @limiter.limit("5/second")
        async def jgl_bot_status(request: Request):
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        "http://jglbotapi.us:83/bot_is_online", timeout=0.1
                    ) as bot_response:
        ...
        async def get_info_for_jgl_bot(request: Request):
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get(
                        "http://jglbotapi.us:83/info", timeout=0.1
                    ) as response:
                ...
                dict = {
                    ...
                    "cogs": cogs,
                    "ping": ping,
                    "size": {"gb": size_gb, "mb": size_mb, "kb": size_kb},
                }
